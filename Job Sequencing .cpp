...
// it return num 0f jobs and profit
   pair <int,int> JobScheduling(Job arr[], int n) {
     sort(arr, arr + n, comparison);   // sort it based on profit
      int max_deadline = arr[0].dead;
      for (int i = 1; i < n; i++) {
         max_deadline = max(max_deadline, arr[i].dead);    // max of all deadlinea of all jobs 
      }
     ...
     // try to schdule the job as alate as possible
     for (int i = 0; i < n; i++) {
